...
const CustomButton = ({
  ...
  } ${extraClasses} ${disabled ? "opacity-50 cursor-not-allowed" : ""}`;
  ...
  if (href) {
    ...
    return (
      <button
        type="button"
        className={"flex justify-center items-center gap-2" + " " + classes}
        disabled={disabled}
        {...props}
      >
        {disabled && (
          ...
          >
            <svg
              aria-hidden="true"
              className="size-8 text-primary animate-spin fill-purple-500"
              viewBox="0 0 100 101"
              fill="none"
